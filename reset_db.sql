IF DB_ID('db_thuvien') IS NOT NULL
BEGIN
    ALTER DATABASE db_thuvien SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE db_thuvien;
    PRINT N'Đã xóa database db_thuvien';
END
GO
...
GO
PRINT N'Đã tạo database db_thuvien mới';
GO

USE db_thuvien;
GO

ALTER DATABASE db_thuvien SET MULTI_USER;
GO
...
BEGIN
    ...
    UPDATE SACH
    SET SoLuongTon = (
        SELECT COUNT(*) FROM BANSAO WHERE BANSAO.ISBN = SACH.ISBN
    )
    WHERE SACH.ISBN IN (
