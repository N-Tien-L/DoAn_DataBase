IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'db_thuvien')
BEGIN
...
-- bảng BANDOC
CREATE TABLE BANDOC (
	IdBD INT IDENTITY(1,1) PRIMARY KEY, -- start = 1, auto +1
	HoTen NVARCHAR(100) NOT NULL,
...
CREATE TABLE PHIEUMUON (
	...
	NgayMuon DATE NOT NULL DEFAULT GETDATE(), -- default lấy ngày hiện tại
	HanTra DATE NOT NULL,

	FOREIGN KEY (IdBD) REFERENCES BANDOC(IdBD)
	ON DELETE NO ACTION ON UPDATE CASCADE,
	-- không cho phép xóa bạn đọc khi họ đang mượn sách
	-- IdBD của một bạn đọc update -> IdBD tương ứng trong PHIEUMUON update theo 
...
ON BANSAO
AFTER INSERT, UPDATE, DELETE -- UPDATE trường hợp update ISBN của bản sao trỏ tới sách khác (hiếm)
AS
BEGIN
    SET NOCOUNT ON;

    -- Cập nhật số lượng tồn cho các ISBN bị ảnh hưởng
    UPDATE SACH
    SET SoLuongTon = (
        SELECT COUNT(*)
        FROM BANSAO
